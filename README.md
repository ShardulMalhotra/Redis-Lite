...
## Topics this covers (for interviews)

**Networking / I/O**
- TCP as a byte stream and why message framing is your problem
- Blocking vs. non-blocking I/O
- The Selector / event-loop model (Node.js, nginx, Redis all use this)

**Data structures**
- `ConcurrentHashMap` internals (bucket locking)
- Intrusive doubly-linked lists for O(1) LRU
- LRU cache algorithm — implemented, not memorized

**Concurrency**
- Race conditions and lock reasoning
- `ReentrantLock` vs `synchronized`
- Lock striping and its trade-offs

**Persistence & durability**
- Write-ahead logging (same idea in databases, Kafka)
- fsync and the durability/performance trade-off
...
> Built a distributed in-memory cache engine in raw Java (NIO, no frameworks) implementing a custom TCP protocol, hand-rolled LRU eviction, TTL expiry, lock-striped atomic operations, and write-ahead-log-based crash recovery. Verified correctness under concurrent load (4000 concurrent increments, zero lost updates) and deployed via Docker with CI on GitHub Actions.
